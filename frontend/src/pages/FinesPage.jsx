...
function FinesPage({ user, toast }) {
  ...
  const totalUnpaid = displayFines
    ...
    try {
      ...
      if (uploadMode === "file") {
        ...
        finalUrl = uploadedPath.startsWith("http")
          ? uploadedPath
          : `${API_BASE}${uploadedPath}`;
      } else {
        finalUrl = proofUrl.trim();
        if (!finalUrl) {
          toast("URL bukti wajib diisi", "error");
          setSubmitting(false);
          return;
        }
        if (!finalUrl.startsWith("http")) {
          toast("URL harus diawali https://...", "error");
          setSubmitting(false);
          return;
        }
      }
  ...
  return (
    <div>
      ...
      <div className="fine-banner">
        <div>
          <div className="fine-banner-val" style={{ color: "var(--c-red)" }}>
            {fmt(totalUnpaid)}
          </div>
          <div className="fine-banner-lbl">Belum Bayar</div>
        </div>
        <div>
          <div className="fine-banner-val">
            {
              displayFines.filter((f) => f.status === "pending_verification")
                .length
            }
          </div>
        ...
        <div>
          <div className="fine-banner-val">
            {displayFines.filter((f) => f.status === "paid").length}
          </div>
      ...
      {isAdmin &&
        displayFines.filter((f) => f.status === "pending_verification").length >
          0 &&
        statusF === null && (
          <div
          ...
          >
            ...
            <div>
              ...
              <button
                ...
                style={{ marginLeft: 10 }}
                onClick={() => setStatusF("pending_verification")}
              >
                Cek Sekarang →
              </button>
            </div>
      ...
      {!isAdmin &&
        displayFines.filter((f) => f.status === "unpaid").length > 0 &&
        statusF === null && (
          <div
          ...
          >
            ...
            <div>
              <strong>
                {displayFines.filter((f) => f.status === "unpaid").length} denda
                keterlambatan
              </strong>{" "}
              belum Anda lunasi! Segera selesaikan pembayaran untuk meminjam
              buku lagi.
            </div>
          </div>
        )}
      {!isAdmin &&
        displayFines.filter((f) => f.status === "rejected").length > 0 &&
        statusF === null && (
          <div
            className="alert alert-error"
            style={{
          ...
          >
            ...
            <div>
              <strong>
                {displayFines.filter((f) => f.status === "rejected").length}{" "}
                pembayaran ditolak
              </strong>{" "}
              oleh admin! Mohon perbaiki bukti transfer Anda.
            </div>
          </div>
        )}
      ...
      {loading ? (
        ...
        <Empty
          ...
          sub={
            statusF ? "Coba filter lain" : "Semua buku dikembalikan tepat waktu"
          }
        />
      ) : (
              ...
              {displayFines.map((f) => {
                ...
                return (
                  <tr key={f.fine_id}>
                    ...
                    <td style={{ fontWeight: 700, color: "var(--c-red)" }}>
                      {fmt(f.amount)}
                    </td>
                    <td>
                        ...
                        {isAdmin && f.status === "pending_verification" && (
                          <>
                            <button
                              ...
                              onClick={async () => {
                                try {
                                  await approveFine(f.fine_id);
                                  toast("Denda ditandai lunas");
                                } catch (err) {
                                  toast(err.message, "error");
                                } finally {
                                  load();
            ...
            <>
              ...
              <button
                ...
                disabled={
                  submitting ||
                  uploading ||
                  (uploadMode === "file" ? !proofFile : !proofUrl.trim())
                }
        ...
        >
          ...
          <div style={{ display: "flex", gap: 4, marginBottom: 16 }}>
            <button
              ...
              style={{
                ...
                border: "1.5px solid",
                borderColor:
                  uploadMode === "file" ? "var(--c-accent)" : "var(--c-border)",
                ...
                background:
                  uploadMode === "file"
                    ...
                    : "var(--c-surface)",
                color:
                  uploadMode === "file" ? "var(--c-accent2)" : "var(--c-text2)",
                cursor: "pointer",
                fontSize: 13,
                fontWeight: 600,
                fontFamily: "inherit",
            ...
            <button
              ...
              style={{
                ...
                border: "1.5px solid",
                borderColor:
                  uploadMode === "url" ? "var(--c-accent)" : "var(--c-border)",
                borderRadius: "var(--r-md)",
                background:
                  uploadMode === "url"
                    ? "var(--c-accentbg)"
                    : "var(--c-surface)",
                color:
                  uploadMode === "url" ? "var(--c-accent2)" : "var(--c-text2)",
                cursor: "pointer",
                fontSize: 13,
                fontWeight: 600,
                fontFamily: "inherit",
          ...
          {uploadMode === "file" ? (
            <Field
              ...
              hint="Format: JPG, PNG, JPEG (maks. 5MB)"
            >
              <div
                style={{
                ...
                onDragLeave={(e) => {
                  e.currentTarget.style.borderColor = proofFile
                    ? "var(--c-green)"
                    : "var(--c-border)";
                }}
                onDrop={(e) => {
              ...
              >
                ...
                {proofFile ? (
                  ...
                  <div>
                    <div
                      style={{ fontSize: 28, marginBottom: 8, opacity: 0.5 }}
                    >
                      📸
                    </div>
                    <div
            ...
            >
              <Input
                ...
                autoFocus
                onKeyDown={(e) => {
                  if (e.key === "Enter" && proofUrl.trim()) doSubmitProof();
                }}
              />
            </Field>
          )}
      ...
      {rejectModal && (
        ...
        >
          <div className="alert alert-warning" style={{ marginBottom: 16 }}>
            Denda #{rejectModal.fine_id} —{" "}
            <strong>{fmt(rejectModal.amount)}</strong>
          </div>
          <Field
            label="Alasan Penolakan *"
            hint="Member akan melihat alasan ini"
          >
            <Textarea
              value={rejectNote}
