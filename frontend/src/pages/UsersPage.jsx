...
function UsersPage({ toast }) {
  ...
  const del = (u) =>
    setConfirm({
      ...
      message: `Yakin hapus "${u.full_name}"?`,
      onConfirm: async () => {
        try {
    ...
    <div>
            ...
            <tbody>
              {users.map((u) => (
                <tr key={u.user_id}>
                  ...
                  <td>
                    <span
                      className={`badge ${u.role === "admin" ? "badge-blue" : "badge-slate"}`}
                    >
                      {u.role === "admin" ? "Admin" : "Member"}
                    </span>
                  </td>
                  <td style={{ color: "var(--c-text3)", fontSize: 12 }}>
        ...
        >
          ...
          >
            ...
            <div>
              ...
              <span
                className={`badge ${detailUser.role === "admin" ? "badge-blue" : "badge-slate"}`}
              >
          ...
          >
            {[
              ...
              <div
                key={label}
                style={{ gridColumn: label === "Email" ? "1 / -1" : undefined }}
              >
                <div
                  style={{
                    fontSize: 10.5,
        ...
        >
          <Field label="Nama Lengkap *">
            <Input
              ...
              autoFocus
              onKeyDown={(e) => {
                if (e.key === "Enter" && editForm.full_name.trim()) saveEdit();
              }}
            />
          </Field>
          <Field label="Role">
      ...
      {addModal && (
        ...
        >
          ...
          <Field
            label="Password *"
            hint="Min. 8 karakter, huruf besar+kecil+angka+spesial (@$!%*?&)"
          >
            <Input
              type="password"
              value={form.password}
      ...
      {resetPwModal && (
        <Modal
          title={`Reset Password: ${resetPwModal.full_name}`}
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  try {
                    ...
                    toast(
                      `Password ${resetPwModal.full_name} berhasil direset`,
                    );
        ...
        >
          ...
          >
            ⚠️ Password baru langsung aktif. Informasikan kepada pengguna yang
            bersangkutan.
          </div>
          <Field
            label="Password Baru *"
                    ...
                    <div className="pw-bars">
                      {[1, 2, 3, 4, 5].map((i) => (
                        <div
                          ...
                          className="pw-bar"
                          style={{
                            background: i <= s.score ? s.color : undefined,
                          }}
                        />
                      ))}
                    </div>
                    <span className="pw-label" style={{ color: s.color }}>
                      {s.label}
                    </span>
                  </div>
                );
      ...
      {confirm && (
        <Confirm {...confirm} danger onCancel={() => setConfirm(null)} />
      )}
    </div>
  );
}
