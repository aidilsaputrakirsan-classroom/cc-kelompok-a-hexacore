...
function TransactionsPage({ user, toast, onNav }) {
  ...
  const countdown = (dueDate) => {
    ...
    if (diff > 0)
      return {
        text: `${diff} hari lagi`,
        color: diff <= 2 ? "#f59e0b" : "#16a34a",
        warning: diff <= 2,
      };
    if (diff === 0)
      return { text: "Hari ini!", color: "#ef4444", warning: true };
    return {
      text: `${Math.abs(diff)} hari terlambat`,
      color: "#ef4444",
      warning: true,
    };
  };
  ...
  const visibleTrxs = trxs.filter(
    (t) => t.status !== "overdue" && t.status !== "lost",
  );
  ...
  const doSubmitBorrow = async () => {
    if (!form.user_id) {
      toast("Pilih pengguna dulu", "error");
      return;
    }
    if (!form.book_id) {
      toast("Pilih buku dulu", "error");
      return;
    }
    ...
    try {
      ...
      toast(
        `Pengajuan dikirim! Jatuh tempo: ${dueDate.toLocaleDateString("id-ID", { day: "numeric", month: "long", year: "numeric" })}`,
      );
      setModal(false);
      load();
    } catch (err) {
  ...
  const doAction = (action, id, label) =>
    setConfirm({
      title: label,
      message: `Konfirmasi ${label.toLowerCase()} transaksi #${id}?`,
      onConfirm: async () => {
        try {
          const res = await action(id);
          if (label === "Kembalikan" && res?.status === "overdue") {
            toast(
              "Catatan: Terlambat dikembalikan. Denda ditambahkan.",
              "error",
            );
  ...
  const STATUS_TABS = [
    ...
    { v: "borrowed", l: "Dipinjam" },
    { v: "returned", l: "Dikembalikan" },
    { v: "overdue", l: "Terlambat" },
  ...
  return (
    <div>
      <div className="page-header">
        ...
        {isAdmin && (
          <button className="btn btn-primary" onClick={openBorrowModal}>
            + Ajukan Pinjam
          </button>
        )}
      </div>
      ...
      <div className="filter-pills" style={{ marginBottom: 20 }}>
        {STATUS_TABS.map((s) => (
          <button
            key={s.v}
            className={`filter-pill${statusF === s.v ? " active" : ""}`}
      ...
      {isAdmin &&
        trxs.filter((t) => t.status === "pending").length > 0 &&
        statusF === "" && (
          <div
          ...
          >
            ...
            <div>
              <strong>
                {trxs.filter((t) => t.status === "pending").length} pengajuan
                peminjaman
              </strong>{" "}
      ...
      {isAdmin &&
        trxs.filter(
          (t) => t.status === "borrowed" && new Date() > new Date(t.due_date),
        ).length > 0 &&
        statusF === "" && (
          <div
          ...
          >
            ...
            <div>
              <strong>
                ...
                buku lewat batas waktu!
              </strong>{" "}
              Harap hubungi peminjam.
            </div>
          </div>
        )}
      ...
      {isAdmin && statusF === "" && (
        ...
        >
          <span style={{ fontSize: 18 }}>💳</span>
          <div>
            Transaksi yang dilaporkan Hilang atau Dikembalikan Terlambat
            otomatis pindah ke
            <button
              className="btn btn-ghost btn-sm"
              style={{ marginLeft: 10 }}
        ...
        statusF === "" && (
          ...
          >
            ...
            <div>
              ...
              kamu sedang menunggu persetujuan admin.
            </div>
          </div>
        )}
      {!isAdmin &&
        ...
        statusF === "" && (
          ...
          >
            ...
            <div>
              ...
              kamu melewati jatuh tempo! Segera kembalikan di bawah ini, atau
              ...
              {visibleTrxs.map((t) => {
                ...
                return (
                  <tr key={t.transaction_id}>
                    ...
                    <td>
                      <div
                        style={{ fontWeight: 600, fontSize: 13, maxWidth: 200 }}
                      >
                        {bookName(t)}
                      </div>
                      <div style={{ fontSize: 11, color: "var(--c-text3)" }}>
                                ...
                                setConfirm({
                                  ...
                                  onConfirm: async () => {
                                    try {
                                      ...
                                      toast(
                                        "Buku dilaporkan hilang. Masuk ke halaman denda.",
                                        "error",
                                      );
                                      setTimeout(() => onNav?.("fines"), 1500);
                                    } catch (err) {
                                  ...
                                  onConfirm: async () => {
                                    try {
                                      ...
                                      if (res?.status === "overdue") {
                                        toast(
                                          "Dikembalikan terlambat! Segera lunasi denda.",
                                          "error",
                                        );
                                        setTimeout(
                                          () => onNav?.("fines"),
                                ...
                                setConfirm({
                                  ...
                                  onConfirm: async () => {
                                    try {
                                      await reportBookLost(t.transaction_id);
                                      toast(
                                        "Berhasil dilaporkan. Silakan bayar di layar Denda.",
                                        "error",
                                      );
                                      setTimeout(() => onNav?.("fines"), 1500);
                                    } catch (err) {
      ...
      {modal && (
        ...
        >
          ...
          <Field
            label="Buku *"
            hint="Ketik nama/pengarang untuk menyaring, atau pilih langsung dari daftar"
          >
            <Input
              value={bookSearch}
              onChange={(e) => {
            ...
            {!form.book_id && (
              ...
              >
                ...
                {filteredBooks.length > 10 && (
                  ...
                  >
                    +{filteredBooks.length - 10} buku lainnya. Ketik lebih
                    spesifik.
                  </div>
                )}
              </div>
