...
function HomePage({ user, onNav, toast }) {
  ...
  const filtered = books.filter((b) => {
    ...
    if (
      filterGenre &&
      !b.genres?.some((g) => g.genre_id === Number(filterGenre))
    )
      return false;
    return true;
  });
  ...
  const doSubmitBorrow = async () => {
    ...
    try {
      ...
      toast(
        `"${borrowModal.title}" berhasil diajukan! Jatuh tempo: ${dueDate.toLocaleDateString("id-ID", { day: "numeric", month: "long", year: "numeric" })}`,
      );
      setBorrowModal(null);
    } catch (err) {
  ...
  const sectionLabel = () => {
    if (filterCat)
      return (
        cats.find((c) => c.category_id === Number(filterCat))?.name || "Koleksi"
      );
    if (filterGenre)
      return (
        genres.find((g) => g.genre_id === Number(filterGenre))?.name || "Genre"
      );
    ...
    <div>
      ...
      <div className="hero">
        <div className="hero-inner">
          ...
          <p className="hero-sub">
            Koleksi lengkap buku perpustakaan digital. Cari, pinjam, dan nikmati
            bacaanmu.
          </p>
          ...
          {genres.length > 0 && (
            <div className="cat-chips">
              <button
                className={`cat-chip${!filterGenre ? " active" : ""}`}
                onClick={clearFilter}
              >
                Semua
              </button>
      ...
      <div className="filter-bar">
        <div className="filter-bar-inner">
          <div className="filter-bar-left">
            ...
            <span
              style={{ fontSize: 12, color: "var(--c-text3)", marginLeft: 8 }}
            >
              {sorted.length} buku
            </span>
            {hasFilter && (
      ...
      {borrowModal && (
        <div
          ...
          onClick={() => !borrowing && setBorrowModal(null)}
        >
          <div
            className="modal"
